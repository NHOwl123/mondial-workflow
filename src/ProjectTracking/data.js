...
const BACKUP_KEY   = "psa-backups";
const VERSION_KEY  = "psa-version";
const MAX_BACKUPS  = 3;

// Increment this number whenever you deploy a breaking data change
...
export async function loadStoredVersion() {
  try {
    const snap = await getDoc(doc(db, "app-data", VERSION_KEY));
    return snap.exists() ? snap.data().version : null;
  } catch { return null; }
}

export async function saveStoredVersion(version) {
  try {
    await setDoc(doc(db, "app-data", VERSION_KEY), { version });
...
export async function loadBackups() {
  try {
    const snap = await getDoc(doc(db, "app-data", BACKUP_KEY));
    return snap.exists() ? (snap.data().backups || []) : [];
  } catch { return []; }
}

export async function saveBackups(backups) {
  try {
    await setDoc(doc(db, "app-data", BACKUP_KEY), { backups });
...
export async function restoreBackup(backupId) {
  ...
  await savePSAData(backup.state);
  return backup.state;
}
...
export const SEED_PROJECTS = [
  {
    id: "proj-1", customerId: "cust-1",
    name: "Hartwell — Phase 1 Go-Live", status: "active",
    leadConsultantId: "usr-2", consultantIds: ["usr-2","usr-1","usr-3"],
    templateId: "tmpl-2", poHours: 200, anticipatedHours: 195, hoursLevel: "category",
    startDate: d(2025,11,1), targetDate: d(2026,3,31), companies: 8, users: 24,
    multiCurrency: true, dedicatedServer: true,
    subscriptionStart: d(2026,4,1), subscriptionEnd: d(2027,3,31),
    milestones: [
  ...
  {
    id: "proj-2", customerId: "cust-3",
    name: "Castello — Mondial Onboarding", status: "active",
    leadConsultantId: "usr-1", consultantIds: ["usr-1","usr-4"],
    templateId: "tmpl-1", poHours: 80, anticipatedHours: 80, hoursLevel: "category",
    startDate: d(2026,1,10), targetDate: d(2026,4,15), companies: 3, users: 10,
    multiCurrency: false, dedicatedServer: false,
    subscriptionStart: d(2026,4,15), subscriptionEnd: d(2027,4,14),
    milestones: [
  ...
  {
    id: "proj-3", customerId: "cust-2",
    name: "Meridian — Consolidation Module", status: "active",
    leadConsultantId: "usr-3", consultantIds: ["usr-3","usr-2"],
    templateId: "tmpl-2", poHours: 160, anticipatedHours: 170, hoursLevel: "category",
    startDate: d(2026,2,1), targetDate: d(2026,6,30), companies: 6, users: 18,
    multiCurrency: true, dedicatedServer: false,
    subscriptionStart: d(2026,7,1), subscriptionEnd: d(2027,6,30),
    milestones: [
  ...
  {
    id: "proj-4", customerId: "cust-4",
    name: "Brightside — Financial Reporting Setup", status: "closed",
    leadConsultantId: "usr-4", consultantIds: ["usr-4","usr-1"],
    templateId: "tmpl-1", poHours: 80, anticipatedHours: 78, hoursLevel: "category",
    startDate: d(2025,7,1), targetDate: d(2025,10,31), companies: 2, users: 8,
    multiCurrency: false, dedicatedServer: false,
    subscriptionStart: d(2025,11,1), subscriptionEnd: d(2026,10,31),
    milestones: [
  ...
  {
    id: "proj-5", customerId: "cust-5",
    name: "Verano — Multi-Entity Rollout", status: "pipeline",
    leadConsultantId: "usr-2", consultantIds: ["usr-2"],
    templateId: "tmpl-2", poHours: 160, anticipatedHours: 160, hoursLevel: "category",
    startDate: d(2026,4,1), targetDate: d(2026,9,30), companies: 5, users: 15,
    multiCurrency: true, dedicatedServer: true,
    subscriptionStart: d(2026,10,1), subscriptionEnd: d(2027,9,30),
    milestones: [
